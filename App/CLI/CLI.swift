...
private final class CLILogRenderer {
  ...
  private func renderLiveView() {
    ...
    let summary =
      "\(startupPrefix)tracks: \(trackEntries.count)  hot: \(violating.count)  ok: \(ok.count)  pids: \(healthyProcesses.count)  logs: \(nonTrackEntries.count)"
    ...
    if !violating.isEmpty {
      detailLines.append(
        style("Hot (showing \(hotDisplay.count)/\(violating.count))", color: .yellow, bold: true))
    ...
    if !healthyProcesses.isEmpty {
      ...
      detailLines.append(
        style("Healthy pids (showing \(shown)/\(healthyProcesses.count))", color: .green, bold: true))
  ...
  private func formatTrack(_ track: TrackLine, hot: Bool) -> String {
    ...
    if let forSeconds = track.forSeconds {
      return
        "\(prefix)\(track.processName) pid=\(track.pid) \(track.metric) \(valueText)/\(thresholdText) \(ratioText) for=\(forSeconds)"
    }
    return
      "\(prefix)\(track.processName) pid=\(track.pid) \(track.metric) \(valueText)/\(thresholdText) \(ratioText)"
  }
